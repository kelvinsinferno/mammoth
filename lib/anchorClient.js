...
/**
 * createProject — creator initialises a new Mammoth project.
 *
 * @param {Program} program
 * @param {Keypair} mintKeypair — fresh mint keypair (caller must generate)
 * @param {object} params
 * @param {object}  params.supplyMode          — e.g. { fixed: {} } or { elastic: {} }
 * @param {BN}      params.totalSupply          — total token supply (base units, 6 decimals)
 * @param {number}  params.publicAllocationBps  — BPS of totalSupply for public escrow
 * @param {number}  params.creatorBps           — creator SOL split BPS
 * @param {number}  params.reserveBps           — reserve SOL split BPS
 * @param {number}  params.sinkBps              — sink SOL split BPS
 * @param {BN|null} params.launchAt             — Unix timestamp (BN) to lock open_cycle until, or null for immediate
 * @param {PublicKey} params.tokenProgram
 * @param {PublicKey} params.associatedTokenProgram
 * @param {PublicKey} params.systemProgram
 * @param {PublicKey} params.rent
 */
export async function createProject(
  ...
  const protocolTreasuryToken = getAssociatedTokenAddressSync(mintPubkey, protocolTreasury, true);
  const creatorToken = getAssociatedTokenAddressSync(mintPubkey, program.provider.publicKey, false);
  const projectEscrowToken = getAssociatedTokenAddressSync(mintPubkey, projectState, true);
  ...
  const tx = await program.methods
    .createProject(
      ...
      reserveBps,
      sinkBps,
      launchAt,  // Option<i64>: BN timestamp or null
      operatorType  // OperatorType enum: { human: {} } | { aiAssisted: {} } | { aiAutonomous: {} }
    )
    .accounts({
      mint: mintPubkey,
      projectState,
  ...
  return { tx, projectState, mintPubkey };
}

/**
 * activateCycle — permissionless.
 * Transitions a cycle from RightsWindow → Active once rights_window_end has passed.
 *
 * @param {Program} program
 * @param {PublicKey} mintPubkey
 * @param {number} cycleIndex — index of the cycle to activate (0-based)
 */
export async function activateCycle(program, mintPubkey, cycleIndex) {
  const [projectState] = getProjectStatePDA(mintPubkey);
...
/**
 * createHolderRights — creator-only.
 * Allocates rights to a specific holder during the RightsWindow.
 *
 * @param {Program} program
 * @param {PublicKey} mintPubkey
 * @param {number} cycleIndex
 * @param {PublicKey} holderPubkey — the holder to allocate rights to
 * @param {BN} rightsAmount — token amount (base units)
 * @param {BN} expiry — unix timestamp (seconds)
 */
export async function createHolderRights(program, mintPubkey, cycleIndex, holderPubkey, rightsAmount, expiry) {
  const [projectState] = getProjectStatePDA(mintPubkey);
  const [cycleState] = getCycleStatePDA(projectState, cycleIndex);
  const [holderRights] = getHolderRightsPDA(cycleState, holderPubkey);

  const tx = await program.methods
    .createHolderRights(holderPubkey, rightsAmount, expiry)
    .accounts({
      projectState,
      cycleState,
      holderRights,
...
export function parseTransactionError(error) {
  ...
  if (msg.includes('6014') || msg.includes('RightsWindowStillOpen')) return 'Rights window is still open — cannot activate yet';
  if (msg.includes('6015') || msg.includes('LaunchTimeNotReached')) return 'Scheduled launch time has not been reached yet';
  ...
  // DEBUG: surface the raw error message so we can actually see what's failing
  // instead of the useless "Transaction failed, please try again" fallback
  const short = (msg || '').split('\n')[0].slice(0, 300);
  return short ? `Tx failed: ${short}` : 'Transaction failed, please try again';
}
