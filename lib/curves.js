  ...
  if (form.startPrice != null) {
    try {
      ...
      console.log('[mammoth deployProject] chaining openCycle for mint:', mintAddress,
        'activatesAt:', scheduledInFuture ? launchAtUnix : null);
      const cycleResult = await client.openCycle(mintAddress, {
        ...
        // On-chain scheduled activation. Program rejects buy/exercise/claim
        // until clock >= activatesAt, then activate_cycle is permissionless
        // so a keeper (or first visitor) opens the cycle at T with no
        // further signature required from the creator.
