...
export async function deployProject({ connection, walletAdapter, form }) {
  ...
  // Launch a token = open its first cycle. Project + cycle are distinct
  // on-chain objects; without this second call, buyers see "no active cycle"
  ...
  if (form.startPrice != null) {
    try {
      ...
      const rightsWindowDuration = form.rightsRequired ? (form.rightsWindowHours || 24) * 3600 : 60;

      console.log('[mammoth deployProject] chaining openCycle for mint:', mintAddress);
      const cycleResult = await client.openCycle(mintAddress, {
        curveType,
        supplyCap,
      ...
      // Project is already on-chain — don't unwind. Surface the cycle failure
      // so the caller can prompt the user to open a cycle manually.
      console.error('[mammoth deployProject] openCycle failed:', e);
      cycleError = e;
    }
  }
